...
| 多轮对话 | 前端发最近 5 轮 history → 后端拼 LLM 上下文；多轮 few-shot 兜底指代；上轮工具结果（商品列表 id/标题/价格/成色/描述）随 assistant 消息记忆 | ✅ 已上线 |
| 商品增量同步 | 发布/编辑 → Python 增量 upsert；下架/删除 → 删向量；失败静默不影响主流程 | ✅ 已上线 |
| 前端助手交互 | 右下角可拖拽的透明小狗悬浮球（默认/说话/拖拽三态切换 + 悬停提示框）→ 点击滑出右侧 420px 抽屉聊天面板；聊天顶栏放入狗图标并做圆角 | ✅ 已上线 |

**仍待补的短板 / 待提升项：**

- 会话持久化（后端 sessions + Redis）：当前历史存前端 Vue 内存，刷新页面即丢；升级为 `session_id` + Redis 后可恢复跨刷新会话
- 检索向量库需二阶段 Rerank（"滑板"混瑜伽垫、"耳机"混显示器仍需 LLM 硬剔除）
- 匹配 Agent（求购撮合）未做
- 反馈闭环（AI 推荐卡片点击/收藏/下单埋点，沉淀正负样本，可反向调 sort_weight）
- AI 追问增强（"为什么推荐这个""和上一个有什么区别"，需结合商品详情）
...
| 前端 Vue 3 | 右侧抽屉聊天 | `el-drawer`（420px，rtl）替代整页跳转，点击右下角悬浮球滑出，keep-alive 保留返回记录 |
| 前端 Vue 3 | 可拖拽小狗悬浮球 | 透明 PNG 三态切换（默认 `dog-idle` / 说话 `dog-speaking` / 拖拽 `dog-active`），拖拽 `grabbing` 光标 + 放大阴影反馈，悬停白色圆角提示框 |
| 前端 Vue 3 | 聊天顶栏装饰 | 顶栏左侧放小狗图标（与悬浮球图标一致）、上两角做圆角，增强趣味性 |
...
7. 多轮：①「我要个平板，3000以内」→ ②「有更便宜的吗」→ query 正确延续"平板"，price_asc 排序（few-shot 兜底指代消解）
8. 工具结果记忆：① 搜完平板 → ②「第一个是什么成色」→ 精确答 iPad Air 九成新，不再重新检索
9. 增量同步：发布新商品后 AI 立即可搜到；下架后 AI 搜不到

**待补升级项**（均为优化提升，非当前必须）：

| 升级项 | 说明 | 优先级 |
|---|---|---|
| 会话持久化（后端 sessions + Redis） | 现历史存前端，刷新即丢；改 `session_id` + Redis 可跨刷新恢复会话，也为用户画像打底 | 中 |
| 二阶段 Rerank | BGE-Reranker-v2-m3 精排，减少"滑板→瑜伽垫"类语义混淆 | 中 |
| 反馈闭环 | AI 推荐卡片点击/收藏/下单埋点，沉淀正负样本反向调 sort_weight | 低 |
| AI 追问增强 | "为什么推荐这个""和上一个有什么区别"，结合商品详情深度回答 | 低 |
| 匹配 Agent | 发布求购后自动撮合在售商品，推送消息给买卖双方 | 未排期 |
